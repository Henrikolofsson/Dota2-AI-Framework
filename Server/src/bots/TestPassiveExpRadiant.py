...
# Tests if passive exp works properly.
# This bot is run in conjunction with TestPassiveExpDire as the bot
# for the Dire team.

# Testing procedure:
# 1. Radiant and Dire team both move to mid.
# 2. If a hero dies, it stays in base permanently.
# 3. Heroes that are no longer in mid should no longer receive exp from creep kills that happen at the location where
# they died (this is the behaviour that might be bugged).
class TestPassiveExpRadiant(BaseBot):
